...
describe('modules/datasource/deb/packages', async () => {
  let downloadedPackageFile: string;
  const fixtureInRelease = await readFile(
    Fixtures.getPath('InRelease'),
    'utf-8',
  );
  ...
  const fixtureInReleaseBookworm = await readFile(
    Fixtures.getPath('InReleaseBookworm'),
    'utf-8',
  );
  const fixtureInReleaseInvalid = await readFile(
  ...
  describe('downloadPackageFile', () => {
    ...
    it('should not download if the package file has not been modified', async () => {
      // simulate existing cached package file
