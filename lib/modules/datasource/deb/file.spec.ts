...
describe('modules/datasource/deb/file', () => {
  ...
  describe('extract', () => {
    ...
    it('should throw for unknown compression', async () => {
      await expect(
        extract(packageArchiveCache, 'zip', extractedPackageFile),
      ).rejects.toThrowError("Unsupported compression standard 'zip'");
    });
  });
});
